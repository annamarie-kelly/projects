...
public class Board extends JPanel implements ActionListener {
	...
	public int boardHeight() {
		return (int) (getSize().getHeight() - BOARD_HEIGHT) / BOARD_HEIGHT;
	}
	
	public int boardWidth() {
		return (int) (getSize().getWidth() - BOARD_WIDTH) / BOARD_WIDTH;
	}
	...
	public void newPiece() {
		...
		curY = BOARD_HEIGHT + 20;// + curPiece.minY();
		
		if(!tryMove(curPiece, curX, curY - BOARD_HEIGHT)) {
	...
	private void drawSquare(Graphics g, int x, int y, Tetromino shape) {
		...
		g.setColor(color.darker());
		g.drawRect(x + 3, y + 3, squareWidth() - 6, squareHeight() - 6);
		g.drawRect(x + 4, y + 4, squareWidth() - 8, squareHeight() - 8);
		g.setColor(color.brighter());
		g.fillRect(x + 5, y + 5, squareWidth() - 10, squareHeight() - 10);
	}
	...
	public void paint(Graphics g) {
		...
		for(int i = BOARD_WIDTH*BOARD_WIDTH; i > 0; i--) {
			for(int j = 0; j < BOARD_HEIGHT*BOARD_HEIGHT; j++) {
				g.drawLine(j*(boardWidth()+1), BOARD_HEIGHT*BOARD_HEIGHT, j*(boardWidth() + 1), 0);
			}
			g.drawLine(BOARD_WIDTH*BOARD_WIDTH*BOARD_WIDTH*BOARD_WIDTH, i*(boardHeight()), 0, i*(boardHeight()));
		}
